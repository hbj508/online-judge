...
def validate_user_id(form, field):
    ...
    """
    id_query = dbSession.query(User).filter_by(id=field.data).all()
    if id_query:
...
def validate_email(form, field):
    ...
    """
    email_query = dbSession.query(User).filter_by(email=field.data).all()
    if email_query:
...
class RegistrationForm(Form):
    ...
    id = StringField('Registration Number', [validators.required("*required"),
                                             ...
                                             validate_user_id])
    firstName = StringField('First Name', [validators.required(message="*required"),
                                           validators.Length(max=20, message="too long")])
    lastName = StringField('Last Name', [validators.Length(max=20, message="too long")])
